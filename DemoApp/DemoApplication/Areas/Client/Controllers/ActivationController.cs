...
namespace DemoApplication.Areas.Client.Controllers
{
    ...
    public class ActivationController : Controller
    {
        ...
        [HttpGet("activated/{token}", Name = "client-auth-activate")]
        public async Task<IActionResult> Activated([FromRoute] string token)
        {
            //var user = _dataContext.Users.FirstOrDefault(x => x.UserActivation.Token == token);

            var tokenInDb = await _dataContext.UserActivations.FirstOrDefaultAsync(t => t.ActivationToken == token);
        ...
        //    if (activedToken.ActivationToken != null && activedToken.TokenExpireDate < DateTime.Now)
        //    {
        ...
        //    var contentLink = _userService.GenerateUrl<Guid>("client-activated", new { token = token });
